    ...
    public function store(Request $request)
    {
        ...
        $delivery = DB::transaction(function () use ($request, $so, $method) {
            $delivery = SalesDelivery::create([
                ...
                'courier_name' => $method === 'ambil_toko' ? 'Ambil di Toko' : ($request->courier_name ?: \App\Modules\Sales\Services\SalesDeliveryService::courierLabel($so->shipping_courier_code, $so->shipping_service_name)),
                'shipping_provider' => $method === 'ambil_toko' ? null : ($so->shipping_provider ?: null),
