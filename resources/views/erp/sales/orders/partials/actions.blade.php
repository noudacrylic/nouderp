...
<div class="flex flex-wrap items-center gap-1.5" onclick="event.stopPropagation()">
    ...
    @elseif($so->status === 'confirmed')
        ...
        @if($invStatus !== 'invoiced')
            <a href="{{ route('sales.invoices.create', ['sales_order_id' => $so->id]) }}" title="Buat Invoice"
                class="{{ $btnClass }} bg-blue-600 hover:bg-blue-700 text-white border-none">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                @if($showText) <span>{{ $invStatus === 'partial' ? 'SISA INVOICE' : 'BUAT INVOICE' }}</span> @endif
            </a>
        @else
            <div class="{{ $btnClass }} bg-green-100 text-green-700 border-none cursor-default" title="Invoice Selesai">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @if($so->status !== 'void')
        <a href="{{ route('sales.orders.print', $so->id) }}" title="Cetak Sales Order"
            ...
            @if($showText) <span>PRINT</span> @endif
        </a>
    @endif
