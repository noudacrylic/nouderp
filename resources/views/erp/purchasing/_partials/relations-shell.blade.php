        ...
        @if($canVoid !== null)
            @if($canVoid)
                <span class="px-2 py-1 rounded text-xs font-bold bg-green-100 text-green-700">
                    &#10003; Aman di-void &mdash; tidak ada keterkaitan aktif
                </span>
            @else
                <span class="px-2 py-1 rounded text-xs font-bold bg-red-100 text-red-700">
                    &#10007; Tidak bisa di-void &mdash; masih ada {{ $blockingCount }} dokumen aktif
                </span>
            @endif
        @endif
    </div>
                        ...
                        @foreach($group['items'] as $it)
                            <div class="flex items-center justify-between gap-3 text-sm">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    @endif
                                    @if(!empty($it['date']))
                                        <span class="text-xs text-gray-400 whitespace-nowrap">&middot; {{ $it['date'] }}</span>
                                    @endif
                                    @if(!empty($it['extra']))
                                        <span class="text-xs text-gray-500 whitespace-nowrap">&middot; {{ $it['extra'] }}</span>
                                    @endif
                                </div>
