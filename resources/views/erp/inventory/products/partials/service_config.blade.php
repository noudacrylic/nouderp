<div class="bg-white shadow rounded-lg border border-gray-100 flex flex-col h-full">
    ...
    <form action="{{ route('inventory.products.update-info', $product->id) }}" method="POST"
        ...
        <div class="space-y-4 flex-1">
            <label class="block text-xs font-bold text-gray-500 uppercase mb-2">AKUN PENDAPATAN JASA</label>
            <select name="revenue_account_id"
                ...
                @foreach ($accounts as $acc)
                    <option value="{{ $acc->id }}" {{ $product->revenue_account_id == $acc->id ? 'selected' : '' }}>
                        {{ $acc->code }} - {{ $acc->name }}
                    </option>
                @endforeach
            </select>
