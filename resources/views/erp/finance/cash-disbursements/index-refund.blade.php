...
<form method="GET" class="bg-white rounded shadow p-3 mb-3 flex gap-3 items-end text-sm flex-wrap">
    ...
    <div>
        ...
        <a href="{{ route('finance.cash-bank.disbursements.refund') }}" class="text-gray-500 ml-1 text-xs">Reset</a>
    </div>
</form>
        ...
        <tbody>
            @forelse($items as $cd)
                ...
                <tr class="border-b hover:bg-blue-50 cursor-pointer" data-href="{{ route('finance.cash-bank.disbursements.show', $cd->id) }}">
                    ...
                    <td class="px-3 py-2">{{ $cd->customer->name ?? '-' }}</td>
                        ...
                        <div class="flex gap-1 flex-row-reverse flex-nowrap">
                            ...
                            @if($cd->isDraft())
                                <form method="POST" action="{{ route('finance.cash-bank.disbursements.post', $cd->id) }}" class="inline"
                                      onsubmit="return confirm('Post refund {{ $cd->number }}?')">
                                    @csrf
                            ...
                            @elseif($cd->isPosted() && $cd->canBeVoided())
                                <form method="POST" action="{{ route('finance.cash-bank.disbursements.void', $cd->id) }}" class="inline"
                                      onsubmit="return confirm('Void refund {{ $cd->number }}? Jurnal di-void.')">
                                    @csrf
            ...
            @empty
                <tr><td colspan="7" class="px-3 py-6 text-center text-gray-400">Belum ada refund customer.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
