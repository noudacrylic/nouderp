...
<div class="flex items-center justify-between mb-4">
    <h1 class="text-lg font-semibold">Pemasukan Umum</h1>
    <a href="{{ route('finance.cash-bank.receipts.create', ['type' => 'general']) }}"
       class="bg-blue-600 text-white px-3 py-2 rounded text-sm">+ Pemasukan Umum</a>
</div>

<form method="GET" class="bg-white rounded shadow p-3 mb-3 flex gap-3 items-end text-sm flex-wrap">
    <div>
        <label class="block text-xs text-gray-500 mb-1">Cari</label>
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Nomor / pembayar / keterangan"
    ...
    <div>
        ...
        <a href="{{ route('finance.cash-bank.receipts.general') }}" class="text-gray-500 ml-1 text-xs">Reset</a>
    </div>
</form>
    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b text-gray-600">
            <tr>
                ...
                <th class="px-3 py-2 text-left">Pembayar / Sumber</th>
        ...
        <tbody>
            @forelse($items as $cr)
                ...
                <tr class="border-b hover:bg-blue-50 cursor-pointer" data-href="{{ route('finance.cash-bank.receipts.show', $cr->id) }}">
                    ...
                    <td class="px-3 py-2">{{ $cr->payer ?: '-' }}</td>
                        ...
                        <div class="flex gap-1 flex-row-reverse flex-nowrap">
                            ...
                            @if($cr->isDraft())
                                <form method="POST" action="{{ route('finance.cash-bank.receipts.post', $cr->id) }}" class="inline"
                                      onsubmit="return confirm('Post pemasukan {{ $cr->number }}?')">
                                    @csrf
                            ...
                            @elseif($cr->isPosted() && $cr->canBeVoided())
                                <form method="POST" action="{{ route('finance.cash-bank.receipts.void', $cr->id) }}" class="inline"
                                      onsubmit="return confirm('Void pemasukan {{ $cr->number }}? Jurnal di-void.')">
                                    @csrf
            ...
            @empty
                <tr><td colspan="7" class="px-3 py-6 text-center text-gray-400">Belum ada pemasukan umum.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
