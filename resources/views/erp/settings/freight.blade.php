...
<div class="max-w-4xl mx-auto">
    ...
    {{-- REKONSILIASI SALDO BITESHIP --}}
    <div class="bg-white shadow rounded-lg border p-6 mt-6">
        <h2 class="text-lg font-bold text-gray-800 mb-1">Rekonsiliasi Saldo Biteship <span class="text-xs font-normal text-gray-400">(pengaman/opsional)</span></h2>
        <p class="text-sm text-gray-500 mb-4">
            Kalau kamu sudah rutin upload laporan biaya API (di Kas &amp; Bank), saldo buku biasanya sudah pas — panel ini hanya untuk <b>menutup sisa selisih</b> bila ada. Samakan saldo buku dengan saldo asli di dashboard Biteship; selisihnya dicatat ke <b>Beban API Biteship</b>.
        </p>

        <div class="flex items-center gap-6 mb-4">
            <div>
                <div class="text-xs text-gray-500">Saldo Biteship (buku)</div>
                <div class="text-xl font-bold text-gray-800">Rp {{ number_format($biteshipBook ?? 0, 0, ',', '.') }}</div>
            </div>
        </div>

        <form method="POST" action="{{ route('settings.freight.reconcile') }}"
              class="flex flex-wrap items-end gap-3"
              onsubmit="return confirm('Catat selisih saldo Biteship ke Beban Layanan? Pastikan saldo asli sudah benar.')">
            @csrf
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Saldo Asli di Biteship (Rp)</label>
